...
def predict():
    try:
        ...
        # Create masks WITHOUT boolean indexing to avoid error
        h, w = 224, 224
        ...
        # Create overlay SAFELY without boolean indexing
        img_resized = cv2.resize(img, (224, 224))
        overlay = img_resized.copy()
        
        # Use cv2.addWeighted to avoid boolean indexing errors
        disc_colored = np.zeros_like(overlay)
        disc_colored[:] = [0, 255, 0]  # Green
        cup_colored = np.zeros_like(overlay)
        cup_colored[:] = [0, 0, 255]    # Red
        
        # Apply masks using cv2 methods
        disc_mask_3d = cv2.cvtColor(disc_mask, cv2.COLOR_GRAY2BGR)
        cup_mask_3d = cv2.cvtColor(cup_mask, cv2.COLOR_GRAY2BGR)
        
        disc_mask_3d = disc_mask_3d / 255.0
        cup_mask_3d = cup_mask_3d / 255.0
        
        overlay = overlay * (1 - disc_mask_3d) + disc_colored * disc_mask_3d
        overlay = overlay * (1 - cup_mask_3d) + cup_colored * cup_mask_3d
...
if __name__ == "__main__":
    print("🚀 REFUGE Server Started - NO BOOLEAN INDEXING ERRORS!")
